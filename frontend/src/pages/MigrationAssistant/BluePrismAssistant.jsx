...
const BP_CONFIG = {
    placeholder: "Ask about Blue Prism migration or describe your process...",
    welcomeTitle: "Blue Prism to AutomationEdge Migration",
    welcomeSubtitle: "Upload your Blue Prism process files (XML, BPRELEASE). I'll help you map objects into AE DAG workflows.",
};

const FileUploadArea = ({ onFileSelect, onFileRemove, acceptedFiles = ".xml,.bprelease,.object" }) => {
        ...
        <Box sx={{ position: 'relative' }}>
                ...
                <Box sx={{ position: 'absolute', bottom: '100%', left: 0, mb: 2, width: 320, maxHeight: 150, overflowY: 'auto', background: 'rgba(10,22,40,0.95)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '8px', zIndex: 20, p: 0.5 }}>
                    {files.map((file) => (
                        <Box key={file.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1, mb: 0.5, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '6px' }}>
                            <InsertDriveFileIcon sx={{ color: '#8fa3c0' }} />
            ...
            <Menu anchorEl={anchorEl} open={open} onClose={handleClose} PaperProps={{ sx: { background: '#112240', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '10px', mt: 1 } }}>
                <MenuItem onClick={() => fileInputRef.current?.click()} sx={{ color: '#e8edf5', '&:hover': { background: 'rgba(242,101,34,0.18)' } }}>
                    ...
                    <ListItemText primary="Upload Blue Prism Files" secondary="XML, BPRELEASE" secondaryTypographyProps={{ sx: { color: '#8fa3c0', fontSize: '11px' } }} />
                </MenuItem>
                ...
                {message.jsonData && (
                    <Box sx={{ mt: 1, position: 'relative', background: '#0a1628', borderRadius: 1, p: 1.5, border: '1px solid #1a3460', width: '100%' }}>
                        <Tooltip title={copied ? "Copied!" : "Copy Details"}>
                            <IconButton size="small" onClick={handleCopy} sx={{ position: 'absolute', top: 8, right: 8, color: copied ? '#4caf82' : '#8fa3c0', background: 'rgba(255,255,255,0.05)', '&:hover': { color: '#fff', background: 'rgba(255,255,255,0.1)' } }}><ContentCopyIcon sx={{ fontSize: 16 }} /></IconButton>
                        </Tooltip>
...
export default function BluePrismChatWindow({ tool, onBack }) {
    const { token } = useAuth();
    ...
    const handleSend = async (text) => {
        ...
        if (uploadedFiles.length > 0) {
            ...
            if (!content) finalContent = `📌 Uploaded file: ${uploadedFiles[0].name}`;
        }
        ...
        try {
            if (uploadedFiles.length === 0) {
                setTimeout(() => {
                    setIsTyping(false);
                    setMessages((prev) => [...prev, { role: "assistant", content: "Please attach a Blue Prism file for analysis.", time: now() }]);
                }, 1000);
                return;
            }
        ...
        <Box sx={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", animation: "chatEnter 0.25s ease" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, px: 2.5, py: 1.6, borderBottom: "1px solid rgba(255,255,255,0.07)", background: "rgba(10,22,40,0.5)", backdropFilter: "blur(10px)", flexShrink: 0 }}>
                ...
                <Box sx={{ flex: 1, display: "flex", alignItems: "center", gap: 1.5 }}>
                    <Typography sx={{ fontFamily: "'Syne', sans-serif", fontSize: "15px", fontWeight: 700, color: "#fff" }}>Blue Prism to AE Migration</Typography>
                </Box>
            </Box>
            ...
            <Box sx={{ flex: 1, overflowY: "auto", px: 2.5, py: 3, display: "flex", flexDirection: "column", gap: 2 }}>
                {showWelcome && messages.length === 0 && (
                    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 1.5, flex: 1, justifyContent: "center", maxWidth: 560, mx: "auto" }}>
                        <Typography sx={{ fontFamily: "'Syne', sans-serif", fontSize: "22px", fontWeight: 700, color: "#fff", letterSpacing: "-0.4px" }}>{BP_CONFIG.welcomeTitle}</Typography>
                        <Typography sx={{ fontSize: "13.5px", color: "#8fa3c0", lineHeight: 1.6, maxWidth: 400 }}>{BP_CONFIG.welcomeSubtitle}</Typography>
                    </Box>
                )}
            ...
            <Box sx={{ px: 2.5, pt: 1.8, pb: 2, borderTop: "1px solid rgba(255,255,255,0.07)", background: "rgba(10,22,40,0.5)", backdropFilter: "blur(10px)", flexShrink: 0, display: "flex", flexDirection: "column", gap: 0.8 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.2, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", px: 1.5, py: 0.5, "&:focus-within": { borderColor: "rgba(242,101,34,0.45)", boxShadow: "0 0 0 3px rgba(242,101,34,0.18)" } }}>
                    ...
                    <TextField multiline maxRows={4} fullWidth value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder={BP_CONFIG.placeholder} variant="standard" InputProps={{ disableUnderline: true, sx: { fontSize: "13.5px", lineHeight: 1.55, pt: 0.5, pb: 0.5 } }} sx={{ flex: 1, "& .MuiInputBase-root": { color: "#e8edf5", width: "100%" } }} />
